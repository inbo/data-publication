...
/****** Object:  View [ipt].[vwGBIF_INBO_visschade_pompgemalen_events]    Script Date: 25/01/2019 16:44:30 ******/
SET ANSI_NULLS ON
GO
...
2019-01_25  pompgemalen
*********************************/

CREATE View [ipt].[vwGBIF_INBO_visschade_pompgemalen_events]
AS
SELECT 
	...
	, [datasetName] = 'Fish damage by Water pump stations'
...
--	, [dynamicProperties] = N'{"projectName":"' + S.ITEM_NAME + '"}'
	...
	, [samplingProtocol] = 
		CASE CONVERT(Nvarchar(500),ST.SHORT_NAME)
			...
			WHEN 'Schietfuik' THEN 'fyke'
			WHEN 'Gemaalnet' THEN 'pump station net'
			ELSE ST.SHORT_NAME
		END 
...
WHERE
	S.[ITEM_NAME] IN ('Visschade pompgemalen') 
