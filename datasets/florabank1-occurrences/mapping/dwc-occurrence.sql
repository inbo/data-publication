...
/****** Object:  View [ipt].[vwGBif]    Script Date: 21/09/2016 12:54:38 ******/
SET ANSI_NULLS ON
GO
...
SELECT  
	    ...
	    , [verbatimCoordinateSystem] = CASE
										WHEN LEN([tIH].[IFBL_CDE]) > 5 THEN 'IFBL 1x1km'
										ELSE 'IFBL 4x4km'
									   END
		...
		, [georeferenceRemarks] = CONVERT(Nvarchar(100),'coordinates are centroid of 1x1km IFBL square')
	...
	FROM [dbo].[tblWaarneming] tW 
		...
		INNER JOIN [NBNData].[dbo].TAXON_VERSION TTV on TTV.TAXON_VERSION_KEY = NS.INBO_TAXON_VERSION_KEY_FOR_RECOMMENDED_NAME
		INNER join [NBNData].[dbo].TAXON TA ON TA.TAXON_KEY = TTV.TAXON_KEY
