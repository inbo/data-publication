...
This document describes how our bird tracking data are standardized to [Darwin Core](https://en.wikipedia.org/wiki/Darwin_Core), so these can be published via [our IPT](http://data.inbo.be/ipt) to the [Global Biodiversity Information Facility (GBIF)](http://www.gbif.org/).
...
These steps are done by the [bird-tracking-etl](https://github.com/inbo/bird-tracking-etl) (R package) which runs on [AWS](https://github.com/inbo/bird-tracking-aws):
...
We need the processed data for the further steps, because we want the metadadta, be able to filter on outliers (`calc_outlier`) and species (`species_code`), and make use of the calculated field `calc_time_diff`.
...
The following data transformation script was initially created in [Exploratory](https://exploratory.io/), but can be adapted as it is just an R script. Here I describe how to run it in Exploratory, but with minor adaptions I should be able to run elsewhere.
