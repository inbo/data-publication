...
SELECT --S.ITEM_NAME as SurveyName
	...
	, [georeferenceRemarks] = 
		CASE
			WHEN SA.SPATIAL_REF_QUALIFIER = 'Centrd UTM5 x dgmnte' THEN 'coordinates are centroid of used grid square'
			WHEN SA.SPATIAL_REF_QUALIFIER = 'Centroïd deelgemnte' THEN 'coordinates are centroid of used grid square'
			WHEN SA.SPATIAL_REF_QUALIFIER = 'Centroïd UTM 5 km' THEN 'coordinates are centroid of used grid square'
			WHEN SA.SPATIAL_REF_QUALIFIER = 'Imported' THEN 'coordinates are centroid of used grid square' -- These are also UTM5
			WHEN SA.SPATIAL_REF_QUALIFIER = 'Centroïd UTM 1 km' THEN 
																	CASE
																		WHEN shp1.TAG IS NOT NULL THEN 'coordinates are centroid of used grid square'
																		ELSE ''
																	END
			WHEN SA.SPATIAL_REF_QUALIFIER = 'XY from original rec' THEN ''
            ELSE ''
